...
values()  # возвращает список (объект dict_values) значений всех элементов словаря. my_dict.values()
...
random.randint(a, b)  # принимает два обязательных аргумента a и b и возвращает число из отрезка a b включительно, и возвращает
...
# функции ALL(), ANY(), ZIP(), ENUMERATE()


# фунция all(iterable) принимает итерируемый объект iterable и возвращает False, если в нем хотя бы один элемент == False
# проверка, все ли элементы - истинная


# функция any(iterable)  принимает итерируемый объект iterable и возвращает True, если в нем хотя бы один элемент == True
# проверка, есть ли истинный элемент


# Функция zip(iterable1, iterable2) принимает две коллекции и объединяет элементы с одними индексами в кортеж
# возвращает итератор
# с помощью zip удобно создавать словари, на основании кортежей (ключ, значение ключа )
numbers = [1, 2, 3]
words = ['one', 'two', 'three']
...
# функция enumerate(iterable, start) принимает коллекцию iterable и необязательный аргумент start
# возвращает итератор из кортежей (индекс элемента коллекции, элемент коллекции)


# 15.8 шаг 10
# пример использование генератора вне списка
# вывести True ВСЕ если точки по полученным координатам лежать внутри сферы, иначе False
abscissas = map(float, input().split())  # получаю абсциссы (координаты x) сферы преобразую через map в float
ordinates = map(float, input().split())  # получаю одинаты (y)
applicates = map(float, input().split())  # получаю аппликаты (z)

dots = list(zip(abscissas, ordinates, applicates))  # создаю список кортежей координаток точек

# выражение сферы - x**2 + y**2 + z**2 = R**2
print(all(tu[0]**2 + tu[1]**2 + tu[2]**2 <= 4 for tu in dots))  # если все точки (all()) лежиат в сфере - вывожу True
# конструкция all(генератор(выражения for переменная in последовательность))


# 15.8 шаг 11
# проверка ip на корректность

# проверка через all() и генератор (generator object), который каждый элемент айпишника, разделенного точками заменяет на True или False
# элемент должен состоять из цифр и быть >= 0 и <= 255
#
ip = all(elem.isdigit() and 0 <= int(elem) <= 255 for elem in input().split('.'))

print(ip)


# 15.8 шаг 12
# даются a, b, программа должна вывести числа из диапозона от a до b, котореы без остатка делется на каждую свою цифру

# РЕШЕНИЕ 1
def vse_deliteli(start, end):  # функция принимает два параметра - начало последовательности и конец
    # запускаю filter() на всей последовательности в диапозоне от a до b включительно
    # внутри использую функцию, которая принимает число из последовательности, и проверяется условие, где все цифры числа - его делители без остатка
    # конструкция filter(lambda, range)
    # внутри анонимной функции вкладываю all(map(lambda, str(число из последовательности)))
    # lambda принимает аргумент - цифру исходного числа и проверяет, делится ли число на эту цифру без остатка, если да, то True
    # all(проверяет, все ли цифры числа деляет его без остатка)
    return filter(lambda num: all(map(lambda num1: int(num1) != 0 and num % int(num1) == 0, str(num))), range(start, end))


a, b = int(input()), int(input())  # получаю a, b

print(*vse_deliteli(a, b))  # вывожу распокованный итератор


# РЕШЕНИЕ 2 (БОЛЕЕ ЧИТАБЕЛЬНОЕ, ПРЕДПОЧТИТЕЛЬНОЕ)
def check_number(num):
    if '0' in str(num):  # если 0 в числе - оно не может делиться на ноль, число не подходит, возвращаю False
        return False
    # использую число, которое нужно проверить, как последовательность str()
    # из него получаю цифры - элементы последовательность
    # если исходное число делится на цифру - возвращаю True
    # использую all() чтобы проверить, что ВСЕ цифры прошли проверку и вернули True
    return all(map(lambda digit: num % int(digit) == 0, str(num)))


a, b = int(input()), int(input())  # принимаю a, b

# использую filter(), оставляю только числа из полученного диапозона, которые прошли через функцию check_numbers
print(*filter(check_number, range(a, b + 1)))


# 15.8 шаг 13
# проверка пароля на условия:
# есть большая и маленькая буквы
# есть цифры
# длина пароля >= 7
def is_password_good(password):
    upper = any(symb.isalpha() and symb.upper() == symb for symb in password)  # если есть хоть один буквенный символ - в верхнем регистре - True
    lower = any(symb.isalpha() and symb.lower() == symb for symb in password)  # если есть хоть один буквенный символ - в нижнем регистре - True
    digit = any(symb.isdigit() for symb in password)  # если есть хоть один символ - цифра - True
    return upper and lower and digit  # если все три переменные True - возвращается True


password = input()

if len(password) >= 7 and is_password_good(password):  # если длина пароля >= 7 и функция вернула True
    print('YES')
else:
    print('NO')
...
# 15.8 шаг 14
# https://stepik.org/lesson/511854/step/14?unit=504046
# проверить, что в каждом классе есть хотя бы один отличник – ученик с оценкой 5
# формат данных:
# 1 строка - кол-во классов
# 2 строка - n-кол-во учеников в классе
# n строк ученик+оценка каждый на отедльной строке
# строка n - кол-во учеников в классе и т.д.
def is_cool_guy_in_class(student_grade):  # как аргумент в функцию передается словарь {ученик: оценка, ученик:оценка}
    # проверяю, что в словаре ученик: оценка есть хотя бы одно значение == 5,
    return any(map(lambda num: num == 5, student_grade.values()))  # обращаяся к значениям словаря и возвращаю True, если значение - 5


classes = list()

# создаю список классов из словарей с учениками (список словарей)
for count_of_classes in range(int(input())):  # кол-во классов - первая полученная строка
    class_dict = dict()  # здесь для каждого класса будет создаваться свой словарь

    for count_of_students_in_class in range(int(input())):  # кол-во учеников в классе
        student_grade = input().split()  # строка "ученик оценка" разбиваю на список [Ученик, оценка]
        class_dict.setdefault(student_grade[0], int(student_grade[1]))  # создаю словарь, где ключ-ученик, значение-оценка ученика

    classes.append(class_dict)  # добавляю сформированный словарь учеников в список с классами

# все элементы списка (словари) преобразую через map, передавая как аргумент, функцию, которая проверяет есть ли в словаре пятерка
if all(map(is_cool_guy_in_class, classes)):  # через all проверяю, что во всех классах (словарях) есть пятерка
    print('YES')
else:
    print('NO')
...
# 16.3 шаг 7
# функцию compose(), которая принимает на вход две других одноаргументных функции f и g и возвращает новую функцию.
...
def compose(func_f, func_g):  # верхняя функци, принимает как рагумент две функции
    def compose_inner(x):  # внутренняя функция, принимает один аргумент
        return func_f(func_g(x))  # возвращает значения func_g, func_f, которым передается аргумент x
    return compose_inner  # возвращает внутреннюю фнукцию, которая примет один аргумент



# 16.3 шаг 8
def arithmetic_operation(operation):  # принимает символ - операцию, которую нужно применить к числу
    operation_dict = {'+': lambda x, y: x+y,  # если плюс - возвращается функция, которая принмает два аргумента и возвращает их сумму
                      '-': lambda x, y: x-y,  # минус - разность двух аргументов
                      '*': lambda x, y: x*y,  # произведение двух элементов
                      '/': lambda x, y: x/y}  # деление - частное двух элементов
    return operation_dict[operation]  # возвращает значение из словаря по ключу - имени операции, которую нужно выполнить



# 16.3 шаг 11
# отсортировать ip по десятичный форме представления
# для перевода ip 192.168.1.2 в десятичную форму:
# 192 * 256**3 + 168 * 256**2  + 1 * 256**1 + 2 * 256**0
def ip_calculate_sum(ip):  # получаю на вход один аргумент - ip
    ip_int_list = map(int, ip.split('.'))   # создаю список чисел на основании ip
    # передаю в map lambda  и 2 последовательности - сами числа ip адреса и степени, которые нужны в формуле, от 3 до 0
    ip_10 = map(lambda num, stepen: num * 256**stepen, ip_int_list, range(3, -1, -1))  # в lambda формула перевода числа ip в десятичное представление
    ip_sum = sum(ip_10)  # вывожу сумму всех чисел ip в десятичном представлении
    return ip_sum  # возвращаю полученную сумму


ip_s = [input() for _ in range(int(input()))]  # создаю список ip адресов

print(*sorted(ip_s, key=ip_calculate_sum), sep='\n')  # вывожу отсортированный список по ключу - функции десятичного представления ip
...
# ОТКРЫТИЕ
# OPEN()
# встроенная функция open - открывает файл
# при присваивании открытого файла в переменную в переменную не попадает содержимое файла. Только ссылка на файл

# файловая_переменная = open(имя_файла, режим_доступа, кодировка)
...
# При указании пути до файла обратный слеш нужно экранировать, то есть
"С:\\Users...\\file_name",
# либо использовать raw string
r"С:\Users...\file_name"
# либо ставить прямой слеш
"С:/Users.../file_name"

# при открытии файлов лучше всего указывать кодировки, например
file = open('info.txt', 'r', encoding='utf-8')


# ЧТЕНИЕ ФАЙЛА
file.read(x)  # – читает все содержимое файла или "x" символов, где x число, необязательный аргумент. Символа перехода строки \n так-же считываются, возвращает строку
file.readline()  # – читает одну строку из файла до '\n', Если считать строку не удалось – достигнут конец файла и больше строк в нем нет - возвращается пустая строка
file.readlines()  # – читает все содержимое файла и возвращает список строк (одна строка — один элемент списка). При этом, каждая строка в списке заканчивается символом переноса строки '\n'


# для обработки бинарных файлов используются свои литералы
...
file.encoding  # Возвращает тип кодировки, с которым открыт файл


# После окончания работы с файлами их нужно закрывать
# для закрытия используется метод close
file.close()
...
file = open('prices.txt', encoding='utf-8')  # открываю файл в кодировке, чтобы сохранить наименования товаров

# создаю список глубиной два через списочные выражение [[наименовария, кол-во, цена], ]
# начинаю цикл по file.readlines, которая возвращает список строк, далее во вложенном списке разделяю строку по элементам
# переводя числовые в int
goods_formated = [[int(atrib) if atrib.isdigit() else atrib for atrib in position.split()] for position in file.readlines()]
print(goods_formated)

order_summ = list(map(lambda order: order[1]*order[2], goods_formated))  # создаю список с суммами заказа (кол-во * цену)
print(order_summ)

all_orders_summ = sum(order_summ)  # получаю сумму всех заказов
...
WRITE()  # файловая_переменная.writе(строковое_значение)
...
WRITELINES()  # файловая_переменная.writеlines(СПИСОК СТРОК)
# можешт записывать в файл несколько строк за раз (список со строками)


PRINT()  # print('строка', file=output)
# так - же при записи в файл через output можно использовать sep и end и все мощность функции print



# 17.4 шаг 9
# записать в файл random.txt 25 случайных чисел, каждое с новой строки
import random

with open('random.txt', 'w') as random_numbers_file:
    # создаю список из 24 чисел, где получаю случайное число из диапозона, перевожу в str и прибавляю \n переход строки
    random_numbers_list = [str(random.randint(111, 777)) + '\n' for _ in range(24)]
    random_numbers_file.writelines(random_numbers_list)  # далее записываю строки (в концке каждой строки - переход на каждую)
    random_numbers_file.write(str(random.randint(111, 777)))  # в конце записываю еще одну строку, уже без перехода на новую строку в конце строки, чтобы не было лишнего пробела


# 17.4 шаг 10
with open('input.txt') as input_file:
    input_file_string_list = input_file.readlines()  # считываю содержимое файла построчно
    # print(input_file_string_list)


with open('output.txt', 'w') as output_file:
    index_n_string_from_input_file = list(enumerate(input_file_string_list, 1))  # создаю список из кортежей (индекс строка из пред. файла)
    formated_index_n_string_from_input_file = [f'{str(tu[0])}) {tu[1]}' for tu in index_n_string_from_input_file]  # создаю новый список, создаваю строки 'индекс) строка'
    print(formated_index_n_string_from_input_file)

    output_file.writelines(formated_index_n_string_from_input_file)  # печатаю полученный список строк 'индекс) строка' в новый файл



# 17.4 шаг 11
# https://stepik.org/lesson/519126/step/11?unit=511575
# доступен текстовый файл class_scores.txt вида: фамилия оценка 0 до 100 включительно.
# Напишите программу для добавления баллов к каждому результату и вывода фамилий и новых результатов тестов в файл new_scores.txt.

with open('class_scores.txt', encoding='utf-8') as class_scores:  # открываю файл
    # создаю список кортежей на основании всех считанных строк вида (Фамилия, Оценка) перевожу оценку в int
    # при использовании split символ \n (переход строки) автоматиечски удаляется
    class_scores_list = [tuple([int(j) if j.isdigit() else j for j in i.split()]) for i in class_scores.readlines()]
    # print(class_scores_list)

with open('new_scores.txt', 'w', encoding='utf-8') as new_scores:  # открываю новый файл для записи
    # создаю список строк вида ['Фамилия Оценка+5', ], если после прибавления 5 баллов результат становится > 100 - результатом становится 100
    # для решения проблемы с превышением 100 баллов использую lambda, где возвращаю 100, если оценка > 100, иначе возвращается оценка + 5
    # деалю '\n'.join, чтобы после каждой строки был переход строки
    new_scores_list = '\n'.join([f'{class_scores_list[i][0]} {(lambda num: 100 if num + 5 > 100 else num + 5)(class_scores_list[i][1])}' for i in range(len(class_scores_list))])
    print(new_scores_list)

    new_scores.writelines(new_scores_list)  # печатаю в файл последнюю строку, обрезая лишний переход строки через rstrip
...
# Напишите программу, которая создает файл output.txt и выводит в него содержимое всех файлов
n = int(input())

with open('output.txt', 'w') as output:  # открываю файл output для вывода текста
    for _ in range(n):  # в диапозоне n (кол-во файлов)
        with open(input()) as file:  # открываю файл по полученному имени
            output.write(file.read())  # записываю в файл вывода (output) все полученный строки
...
# функция по переводу строки в формат HH:MM
def format_to_HH_MM(str_time):  # параметр-строка в формате HH:MM
    return datetime.datetime.strptime(str_time, '%H:%M')  # возвращает форматированную строку

with open('logfile.txt', encoding='utf-8') as log, open('output.txt', 'w', encoding='utf-8') as output:
    # создается словарь формата {Имя: (начало, конец)}, обрезая лишние пробелы в начале и конце, исходная строка файла разбивается по запятым
    users_log = {info.split(',')[0]: tuple(map(str.strip, (info.split(',')[1], info.split(',')[2]))) for info in log}
    print(users_log)

    # списочное выражение, сохраняет только ключи словаря, которые прошли через функцию, предварительно время форматируется
    res = [i for i in users_log if format_to_HH_MM(users_log[i][1]) - format_to_HH_MM(users_log[i][0]) >= datetime.timedelta(hours=1)]
    print(*res, sep='\n')

    output.writelines('\n'.join(res))  # строки выводятся в файл разделенные переходом строки


# 18.1 шаг 6 задание из экзамена
...
# Примечание 2. Части слова, не содержащие запрещенных слов дожны сохранить символы и свой регистр

# Открываю два файла, исходный и со словами для игнора
with open(input()) as file, open('forbidden_words.txt') as ignore:
    ignore_words = ignore.read().split()  # создаю список из заблокированных слов
    words = [i for i in file]  # создаю список из строк исходного файла
    words_lower = list(map(str.lower, words))  # создаю копию исходного файла, но с переводом всего в нижний регистр

    for word in range(len(words_lower)):  # цикл по копии исходного списка
        for ignore_word in range(len(ignore_words)):  # цикл по всем игнор-словам
            if ignore_words[ignore_word] in words_lower[word]:  # если исходное слово содержит игнор-слово
                # заменяю в копии исходного списка по индексу строку, на строку с замененным игнор словом на звездочки
                words_lower[word] = words_lower[word].replace(ignore_words[ignore_word], len(ignore_words[ignore_word]) * '*')

    # цикл (по индексам) по исходному списку строк (не измененому)
    for i in range(len(words)):
        for j in range(len(words[i])):  # цикл (по индексам) по самой строке
            # если в измененном списке (с удаленными игнор-словами) на той же позиции, что в исходном -
            # стоит не звездочка (символ не из заблокированного слова)
            if words_lower[i][j] != '*':
                print(words[i][j], end='')  # то печатаю этот символ, так-как он прошел проверки
            else:
                print('*', end='')  # иначе, если стоит звездочка (символ был удален, находится в блок-списке) - вывожу звездочку



# 18.1 шаг 8 задание из экзамена
# https://stepik.org/lesson/448983/step/8
with open('temp.txt', encoding='utf-8') as functions:
    all_func = [i for i in functions]
    print(all_func)

    no_comments = [all_func[i].strip() for i in range(len(all_func)) if 'def ' in all_func[i] and '# ' not in all_func[i-1]]
